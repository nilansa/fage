...
try:
    import torch
except ModuleNotFoundError:  # Allows pure NumPy audits to run before the ML env is restored.
    torch = None
...
def corrected_loss_weights(
    ...
    if torch is None:
        raise ModuleNotFoundError("corrected_loss_weights requires PyTorch")
    if mode == "none":
